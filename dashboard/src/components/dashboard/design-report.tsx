    ...
    <div className="space-y-5">
      ...
      <div className="bg-d8x-surface border border-d8x-border rounded-lg p-5">
        <div className="flex items-center gap-2 mb-4">
          <Server className="w-4 h-4 text-d8x-blue" />
          <h3 className="text-sm font-semibold">Architecture</h3>
          <span className="text-xs px-2 py-0.5 rounded bg-d8x-blue/15 text-d8x-blue ml-2">
            {arch.pattern?.replace(/_/g, " ")}
          </span>
        </div>

        {arch.rationale && (
          <p className="text-sm text-d8x-text-secondary mb-4">{arch.rationale}</p>
        )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
            {stack.map((s: any, i: number) => (
              <div key={i} className="bg-d8x-background border border-d8x-border rounded-lg p-3">
                <span className="text-[10px] text-d8x-text-tertiary uppercase tracking-wider">{s.category}</span>
        ...
        <div className="bg-d8x-background border border-d8x-border rounded-lg p-4 mt-3">
          <p className="text-[10px] text-d8x-text-tertiary uppercase tracking-wider mb-3">System Architecture</p>
          <div className="flex items-center justify-center gap-2 flex-wrap">
            ...
            <ArrowRight className="w-4 h-4 text-d8x-text-tertiary" />
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-1.5 px-2 py-1 bg-d8x-blue/10 border border-d8x-blue/20 rounded text-xs text-d8x-blue">
        ...
        <div className="bg-d8x-surface border border-d8x-border rounded-lg p-5">
          ...
          <div className="space-y-2">
            {adrs.map((adr: any, i: number) => (
              <div key={i} className="flex items-start gap-3 bg-d8x-background border border-d8x-border rounded-lg p-3">
                <span className="text-[10px] font-mono text-d8x-text-tertiary bg-d8x-border px-1.5 py-0.5 rounded shrink-0 mt-0.5">{adr.id}</span>
                <div className="min-w-0">
                  <p className="text-sm font-medium">{adr.title}</p>
                  <p className="text-xs text-d8x-text-secondary mt-0.5">{adr.decision}</p>
                  {adr.alternatives_considered && (adr.alternatives_considered as string[]).length > 0 && (
                    <div className="mt-1.5 pl-2 border-l-2 border-d8x-border">
                      {(adr.alternatives_considered as string[]).map((alt: string, j: number) => (
                        <p key={j} className="text-[11px] text-d8x-text-tertiary">✗ {alt}</p>
                      ))}
                    </div>
                  )}
                </div>
      ...
      <div className="bg-d8x-surface border border-d8x-border rounded-lg p-5">
        ...
        {/* ER diagram (visual) */}
        <div className="bg-d8x-background border border-d8x-border rounded-lg p-4 mb-4">
          <p className="text-[10px] text-d8x-text-tertiary uppercase tracking-wider mb-3">Entity-Relationship Diagram</p>
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
            {tables.map((t: any, i: number) => (
              <div key={i} className="bg-d8x-surface border border-d8x-border rounded p-2 text-center hover:border-green-500/30 transition-colors">
                <Database className="w-3 h-3 text-green-400 mx-auto mb-1" />
                <p className="text-[11px] font-medium truncate">{t.name}</p>
                <p className="text-[10px] text-d8x-text-tertiary">{t.columns} cols</p>
              </div>
            ))}
          </div>
        </div>
            ...
            <tbody>
              {tables.slice(0, 12).map((t: any, i: number) => (
                <tr key={i} className="border-t border-d8x-border">
                  ...
                  <td className="px-4 py-2 text-center text-xs text-d8x-text-secondary">{t.columns}</td>
                  <td className="px-4 py-2 text-xs text-d8x-text-secondary">{t.purpose}</td>
                </tr>
              ))}
              {tables.length > 12 && (
                <tr className="border-t border-d8x-border">
                  <td colSpan={3} className="px-4 py-2 text-xs text-d8x-text-tertiary text-center">
              ...
              <tbody>
                {endpoints.slice(0, 15).map((ep: any, i: number) => (
                  <tr key={i} className="border-t border-d8x-border">
                    ...
                    {ep.domain && <td className="px-4 py-2 text-xs text-d8x-text-secondary">{ep.domain}</td>}
                  </tr>
                ))}
                {endpoints.length > 15 && (
                  <tr className="border-t border-d8x-border">
      ...
      <div className="bg-d8x-surface border border-d8x-border rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">Strategy</span>
            <p className="text-sm font-medium mt-0.5">{auth.strategy ?? "JWT"}</p>
          </div>
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">Roles</span>
            <p className="text-sm font-medium mt-0.5">{auth.roles ?? 0}</p>
          </div>
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">Permissions</span>
            <p className="text-sm font-medium mt-0.5">{auth.permissions ?? 0}</p>
          </div>
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">MFA</span>
      ...
      <div className="bg-d8x-surface border border-d8x-border rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">Framework</span>
            <p className="text-sm font-medium mt-0.5">{frontend.framework ?? "Next.js"}</p>
          </div>
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">Pages</span>
            <p className="text-sm font-medium mt-0.5">{frontend.pages ?? 0}</p>
          </div>
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">Components</span>
            <p className="text-sm font-medium mt-0.5">{frontend.components ?? 0}</p>
          </div>
          <div className="bg-d8x-background border border-d8x-border rounded-lg p-3">
            <span className="text-[10px] text-d8x-text-tertiary">State Management</span>
...
function ArchNode({ label, sub, icon: Icon, color, small }: { label: string; sub: string; icon: typeof Server; color: string; small?: boolean }) {
  return (
    <div className={`flex flex-col items-center gap-1 ${small ? "px-2 py-1.5" : "px-3 py-2"} bg-d8x-surface border border-d8x-border rounded-lg`}>
      <Icon className={`${small ? "w-3 h-3" : "w-4 h-4"} ${color}`} />
      <span className={`${small ? "text-[10px]" : "text-xs"} font-medium`}>{label}</span>
      <span className="text-[9px] text-d8x-text-tertiary">{sub}</span>
    </div>
  );
}
